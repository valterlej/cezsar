...
#def load_train_test_split(directory="data/splits/ucfTrainTestlist/", split=1):    
#    train_files = [line.split(" ")[0].split("/")[-1][:-4] for line in open(os.path.join(directory, "trainlist0"+str(split)+".txt"), 'r')]
#    test_files =  [line.split("/")[-1][:-5] for line in open(os.path.join(directory, "testlist0"+str(split)+".txt"), 'r')]
#    return train_files, test_files


def semantic_embedding(x, embedder): 
...
def load_class_sentences(dataset_dir, embedder, objects=None, min_len=10, max_len=50, max_sentences_per_file=10, return_json=True):   
        ...
        for p in paragraphs:
            ...
            for i in s:
                ...
                if len(words) >= min_len:
                    sent = preprocess_sentence(i, pad_punctuation=True, only_letters_and_punctuation=True)
